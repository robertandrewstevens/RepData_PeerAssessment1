...
```{r}
activity <- read.csv(paste0("~/", "activity.csv"), header = TRUE, as.is = TRUE)
```
...
1. Commit your completed `PA1_template.Rmd` file to the `master` branch of your git repository
    + You should already be on the `master` branch unless you created new ones
...
3. You will see a list of commits that you have made to this repository
    + The most recent commit is at the very top
